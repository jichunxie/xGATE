...
`pip install .` installs the `xGATE` package (import it as `xGATE.utilities`) plus
its dependencies, which are declared in `setup.py`. These include PyTorch (for VAE
computations), networkx/igraph (for graph operations), and biological tools
(biopython/mygene) for pathway integration.
